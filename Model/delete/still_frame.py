...
maxV = max(sol[ind][0:ne-1])
minV = min(sol[ind][0:ne-1])
...
maxC = max(sol[ind][ne:2*ne-1])
minC = min(sol[ind][ne:2*ne-1])

#rescale flows
maxQ = abs(max(arrows[ind], key=abs))
...
#color bar
# cbar = plt.colorbar(edges, location ='bottom')
